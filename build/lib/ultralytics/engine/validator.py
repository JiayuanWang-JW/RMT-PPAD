        ...
        for batch_i, batch in enumerate(bar):
            ...
            if self.args.speed and batch_i==0:
                ...
                with torch.no_grad():
                    ...
                    # Test for batch_size = 16
                    print('test2: model inferring only')
                    print('inferring images for batch_size 16 for 1000 times...')
                    x = torch.cat([x] * 16, 0).to(device)
                    ...
                    end_time = time.time()
                    elapsed_time = (end_time - start_time) / 1000
                    print(f'{elapsed_time} seconds, {16 / elapsed_time} FPS, @batch_size 16')
                    del x, _
                    torch.cuda.empty_cache()  # free GPU memory
